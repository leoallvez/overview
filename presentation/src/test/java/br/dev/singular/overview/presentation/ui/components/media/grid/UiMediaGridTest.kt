...
class UiMediaGridTest {
    ...
    @Test
    fun `UiMediaGrid should display items when using LazyPagingItems`() = runTest {
        val testDispatcher = UnconfinedTestDispatcher()
        Dispatchers.setMain(testDispatcher)

        try {
            val flow = flowOf(PagingData.from(listOf(fakeMedia)))

            rule.setContent {
                CompositionLocalProvider(LocalInspectionMode provides true) {
                    val lazyPagingItems = flow.collectAsLazyPagingItems()
                    UiMediaGrid(items = lazyPagingItems)
                }
            }
            rule.onNodeWithText(fakeMedia.title).assertExists()
