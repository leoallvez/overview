# Overview Tagging 

**version** 1.0.0

This document provides an overview of the application screens and their corresponding tagging events. Each section below outlines which events are tracked on specific screens, helping to ensure consistent analytics implementation across the app.

## Index
- [Bottom Navigation Bar](bottom_navigation_bar.md)
- [Splash Screen](splash.md)
- [Home Screen](home.md)
- [Select Streaming Screen](select_streaming.md)
- [Search Screen](search.md)
- [Favorites Screen](favorites.md) 
- [Media Details Screen](media_details.md)
- [YouTube Player Full Screen](youTube_player_full_screen.md)
- [Person Details Screen](person_details.md)