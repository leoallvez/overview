...
class GetAllSuggestionsUseCaseTest {
    ...
    private lateinit var getterSuggestion: GetAll<Suggestion>
    ...
    fun setup() {
        ...
        getterSuggestion = mockk()
        sut = GetAllSuggestionsUseCase(getterSuggestion, getterMediaMock)
    ...
    fun `invoke should return sorted active suggestions with media`() = runBlocking {
        // arrange
        coEvery { getterSuggestion.getAll() } returns listOf(
            suggestionMock.copy(order = 2, isActive = true, key = "key1"),
            suggestionMock.copy(order = 1, isActive = true, key = "key2")
        )
        coEvery { getterMediaMock.getAll(any()) } returns listOf(mediaMock)

        // act
        val result = sut.invoke()

        // assert
        coVerify { getterSuggestion.getAll() }
        coVerify(exactly = 2) { getterMediaMock.getAll(any()) }
    ...
    fun `invoke should respect MAX_MEDIA constant`() = runBlocking {
        // arrange
        coEvery { getterSuggestion.getAll() } returns listOf(suggestionMock)
        coEvery { getterMediaMock.getAll(any()) } returns List(MAX_MEDIA * 2) { mediaMock }
    ...
    fun `invoke should exclude suggestions without medias from the result`() = runBlocking {
        // arrange
        coEvery { getterSuggestion.getAll() } returns listOf(suggestionMock, suggestionMock)
        coEvery { getterMediaMock.getAll(any()) } returns emptyList()
        ...
        // assert
        coVerify { getterSuggestion.getAll() }
        coVerify { getterMediaMock.getAll(any()) }
    ...
    fun `invoke should return failure when no active suggestions are found`() = runBlocking {
        // arrange
        coEvery { getterSuggestion.getAll() } returns emptyList()
    ...
    fun `invoke should return failure when repository throws exception`() = runBlocking {
        // arrange
        coEvery { getterSuggestion.getAll() } throws Exception()
