...
interface ICreateSuggestionsUseCase {
    suspend operator fun invoke(vararg models: Suggestion): UseCaseState<Unit>
}
...
class CreateSuggestionsUseCase(
    ...
    override suspend fun invoke(vararg models: Suggestion): UseCaseState<Unit> {
        return runCatching { creator.create(*models) }
            .fold(
